...
// the admin page, or a trusted GHL workflow webhook step — can generate
// these links. Never exposed in the main app.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let discountNote = '';
    // contact / GHL_API_KEY are hoisted to function scope (not just inside
    // the discountMonths block) so the checkout-URL save-back near the end
    // can reuse them without a second lookup.
    let contact = null;
    let GHL_API_KEY;
    ...
    if (discountMonths) {
      ...
      contact = await findGHLContact(clientEmail, GHL_API_KEY, GHL_LOCATION_ID);
      if (!contact) {
    ...
    // --- NEW: save the generated link onto the contact so an automated
    // workflow (e.g. a Day-12 WhatsApp step) can pull it in via a merge
    // field — the JSON response below isn't reachable from a GHL workflow
    // step, only a value saved on the contact is. Only runs when we already
    // looked the contact up above (i.e. discountMonths was set).
    if (contact) {
      await updateGHLContact(contact.id, { discount_checkout_url: checkoutUrl }, GHL_API_KEY);
    }
